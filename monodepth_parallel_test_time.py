...
def process_directory(params):
    ...
    #for image_path in images_to_process:
        ...
        t7 = time.time()
        print('Total Job PROCESSING RESULT Time 7 = {}'.format(t7 - t6))
        # Run model on image
        disp = sess.run(model.disp_left_est[0], feed_dict={left: input_image_lr})

        run_model = time.time()
        print('Total Job PROCESSING RESULT Time run model = {}'.format(t7 - run_model))
        disp_pp = post_process_disparity(disp.squeeze()).astype(np.float32)

        post_processing = time.time()
        print('Total Job PROCESSING RESULT Time post processing = {}'.format(run_model - post_processing))
        output_name = os.path.splitext(os.path.basename(image_path))[0]

        t8 = time.time()
        print('Total Job PROCESSING RESULT Time output name = {}'.format(t8 - post_processing))
        print('Total Job PROCESSING RESULT Time 8 = {}'.format(t8 - t7))
        # Save array of disparity values
        np.save(os.path.join(output_directory, "{}_disp.npy".format(output_name)),
                disp_pp)
        # Save result png's
        t9 = time.time()
        print('Total Job PROCESSING RESULT Time 9 = {}'.format(t9 - t8))
        ...
        t10 = time.time()
        print('Total Job PROCESSING RESULT Time 10 = {}'.format(t10 - t9))
    with ThreadPoolExecutor() as executor:
        disp_images_processed = [executor.submit(process_single_image, i) for i in images_to_process]
        res = [f.result() for f in disp_images_processed]

    
    print('done!')
    print('Total Job PROCESSING RESULT Time 1 = {}'.format(t1 - t0))
    print('Total Job PROCESSING RESULT Time 2 = {}'.format(t2 - t1))
    print('Total Job PROCESSING RESULT Time 3 = {}'.format(t3 - t2))
    print('Total Job PROCESSING RESULT Time 4 = {}'.format(t4 - t3))
    print('Total Job PROCESSING RESULT Time 5 = {}'.format(t5 - t4))
    print('Total Job PROCESSING RESULT Time 6 = {}'.format(t6 - t5))
